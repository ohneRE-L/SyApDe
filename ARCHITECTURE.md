...
| **GET** | `/api/v1/auth/verify` | Проверка валидности JWT-токена и профиль пользователя | Авторизован (любая роль) | `200 OK` |
...
### Механизм аутентификации и проверки JWT-токена

#### 1. Схема передачи токена
Все защищенные эндпоинты системы требуют передачи Bearer-токена в стандартном HTTP-заголовке запроса:
```http
Authorization: Bearer <access_token>
```

#### 2. Структура полезной нагрузки (JWT Claims Payload)
Токен подписывается на сервере алгоритмом HMAC-SHA256 (HS256) либо RS256 и содержит необходимые данные для проверки прав без постоянного обращения к БД:
```json
{
  "sub": "456e4567-e89b-12d3-a456-426614174111",
...
```
* `sub` (Subject) — уникальный UUID пользователя в базе данных.
* `email` — адрес корпоративной почты.
* `role` — роль в системе (`USER`, `SUPPORT_AGENT`, `ADMIN`) для реализации RBAC.
* `iat` (Issued At) — отметка времени выдачи токена.
* `exp` (Expiration Time) — время истечения срока действия токена (TTL = 1 час).

#### 3. Алгоритм проверки JWT (Auth Middleware / Filter)
При каждом запросе к защищенному ресурсу серверный фильтр выполняет следующие шаги:
1. **Проверка заголовка `Authorization`**:
   * Если заголовок отсутствует или не соответствует шаблону `^Bearer\s+(.+)$`, сервер возвращает `401 Unauthorized` с кодом ошибки `MISSING_OR_INVALID_AUTH_HEADER`.
2. **Проверка криптографической подписи**:
   * Выполняется проверка контрольной суммы токена с использованием секретного ключа (`JWT_SECRET`) или открытого ключа. Если подпись скомпрометирована или токен поврежден — возвращается `401 Unauthorized` (`INVALID_SIGNATURE`).
3. **Проверка времени жизни (`exp`)**:
   * Сравнивается текущее системное время с меткой `exp`. Если `current_time >= exp`, возвращается `401 Unauthorized` (`TOKEN_EXPIRED`).
4. **Проверка актуальности пользователя**:
   * По идентификатору `sub` проверяется флаг `is_active` в кэше/БД. Заблокированные пользователи получают `401 Unauthorized` (`USER_DISABLED`).
5. **Ролевая авторизация (RBAC)**:
   * Проверяется соответствие `role` из клеймов минимально требуемым правам эндпоинта. При нехватке прав возвращается `403 Forbidden` (`FORBIDDEN`).
